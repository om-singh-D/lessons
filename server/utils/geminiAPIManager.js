...
class GeminiAPIManager {
  constructor() {
    ...
    logger.info(`Gemini API Manager initialized with ${this.apiKeys.length} API keys`);
  }
  ...
  rotateToNextAvailableKey() {
    ...
    do {
      ...
      if (attempts >= this.apiKeys.length) {
        logger.warn('All API keys are rate limited or blocked');
  ...
  async generateContent(prompt, options = {}) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      try {
        ...
        logger.info(`Successful request with key index ${index}`);
        return response;
  ...
  async handleAPIError(error, keyIndex) {
    ...
    if (this.isRateLimitError(error)) {
      logger.warn(`Rate limit hit for key index ${keyIndex}:`, error.response?.data || error.message);
      ...
      logger.error(`Permanent error for key index ${keyIndex}:`, error.response?.data || error.message);
      // Block this key permanently (until server restart)
      this.blockedKeys.add(keyIndex);
      this.rotateToNextAvailableKey();
      
    } else {
      logger.error(`API error for key index ${keyIndex}:`, error.response?.data || error.message);
    }
  }
...
Format the response as JSON:
{
  ...
  // Reset blocked keys (for testing or manual intervention)
  resetBlockedKeys() {
    ...
    logger.info('All blocked keys have been reset');
  }
}
